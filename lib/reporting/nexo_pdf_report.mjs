...
function writeValuations(writer, deep, includeClassic) {
  ...
  if (layers.length) {
    ...
    if (deep.integridade_analise?.valuation_zone_suppressed) {
      ...
      writer.card(
        "Zona e BESST suspensos pelo servidor",
        "A derivação de uma camada ou a memória estruturada da convergência não passou na validação. O servidor preservou somente os cálculos verificáveis; a zona não foi reconstruída automaticamente.",
        `Camadas afetadas: ${affectedLayers.join(", ") || "não informadas"}.`
      );
    }
  }
  if (classics.length) {
