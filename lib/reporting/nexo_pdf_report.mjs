...
import { evidenceDisplayLabel, evidenceSourceDisplayLabel } from "../ui/evidence_labels.mjs";
...
function writeTdn(writer, result, { showUnavailable = false } = {}) {
  ...
  if (tdn.status !== "ok") {
    if (showUnavailable) {
      ...
      const incompleteWindows = list(tdn.windows).filter((window) => window?.status !== "ok").map((window) => window?.id).filter(Boolean);
      const missingCount = list(tdn.missing).length;
      writer.pair("Status", unclassified ? "TDN ainda não disponível para este ativo" : "Cobertura histórica incompleta");
      writer.paragraph(unclassified
        ? "O ativo ainda não foi incluído na matriz setorial curada do TDN. O restante da análise permanece válido, sem score artificial para este módulo."
        : `As duas janelas obrigatórias ainda não possuem cobertura suficiente${missingCount ? ` · ${missingCount} campos pendentes` : ""}${incompleteWindows.length ? ` · janelas ${incompleteWindows.join(" e ")}` : ""}.`,
      { color: COLORS.muted });
    }
    return;
  }
...
function writeBiblioteca(writer, result) {
  ...
  writer.pair("Documentos no acervo", library.documents_available || 0);
  writer.pair("Documentos indexados", library.documents_indexed || 0);
  writer.pair("Trechos consultados", list(library.chunks_consulted).length);
  writer.pair("Lacunas resolvidas", list(library.lacunas_resolvidas).length);
  writer.pair("Lacunas parciais", list(library.lacunas_parciais).length);
  const partial = new Set(list(library.lacunas_parciais));
  const open = list(library.lacunas_abertas).filter((gap) => !partial.has(gap));
  writer.pair("Lacunas abertas", open.length);
  list(library.documents_used).forEach((id, index) => writer.bullet(`Evidência documental: ${evidenceDisplayLabel(library, id, index)}`));
  list(library.lacunas_parciais).forEach((gap) => writer.card("Lacuna parcialmente respondida", gap, "Há evidência útil, mas ainda falta parte da resposta solicitada."));
  open.forEach((gap) => writer.card("Lacuna documental aberta", gap, "Exige fonte oficial complementar para novo aprofundamento."));
  writer.paragraph("A fonte integral permanece preservada; somente trechos selecionados por lacuna entram no contexto da análise. O servidor valida as referências antes de aceitar ajustes de score.", { color: COLORS.muted });
}
...
function writeValuations(writer, deep, includeClassic) {
  ...
  if (layers.length) {
    ...
    if (deep.integridade_analise?.valuation_zone_suppressed) {
      const affectedLayers = [...new Set([
        ...list(deep.integridade_analise.valuation_corrected_layers),
        ...list(deep.integridade_analise.valuation_invalid_layers),
      ])];
      writer.card(
        "Faixa de preço não consolidada",
        "Menos de duas camadas apresentaram valores utilizáveis. A análise qualitativa permanece válida, mas o desconto aparente não pode ser tratado como margem de segurança sem uma faixa consolidada.",
        affectedLayers.length ? `Camadas a revisar: ${affectedLayers.join(", ")}.` : "Revisar as premissas quantitativas antes de usar a faixa."
      );
    }
  }
  if (classics.length) {
...
function writeDeep(writer, deep, includeClassic) {
  ...
  if (deep.integridade_analise?.version) {
    writer.pair("Motor de integridade", `${asText(deep.integridade_analise.version)} · valuation ${asText(deep.integridade_analise.valuation_version)}`);
  }
  if (Number.isFinite(Number(deep.score_revisado))) {
    ...
    const library = deep?.nexoModules?.BIBLIOTECA || {};
    list(deep.ajustes_score).forEach((item, index) => writer.card(
      `${asText(item?.dimensao)} · ${asText(item?.antes)} para ${asText(item?.depois)}`,
      item?.motivo,
      `Fonte: ${evidenceSourceDisplayLabel(library, item?.fonte_nova, index)}`
    ));
  }
  ...
  if (deep.zona || deep.zona_convergida) {
    ...
    if (deep.integridade_analise?.besst_corrected) {
      const previousBesst = asText(deep.integridade_analise.besst_previous_value);
      writer.card(
        "BESST alinhado à zona",
        /^(?:N\/D|Não calculado)/i.test(previousBesst)
          ? "A análise não trouxe uma faixa BESST utilizável; a faixa foi calculada a partir da zona validada."
          : `Faixa recebida na análise: ${previousBesst}`,
        "O servidor aplicou a regra de 15% a 25% abaixo do piso da zona validada. Esse ajuste não representa, isoladamente, uma nova mudança de fundamento."
      );
    }
    if (!deep.integridade_analise?.valuation_zone_suppressed && list(deep.integridade_analise?.convergence_excluded_layers).length) {
      writer.card(
        "Memória da convergência",
        `Camadas incluídas: ${list(deep.integridade_analise.convergence_included_layers).join(", ")}. Camadas excluídas por justificativa metodológica: ${list(deep.integridade_analise.convergence_excluded_layers).join(", ")}.`,
        deep.integridade_analise.convergence_exclusion_reason || "Exclusão metodológica registrada no Deep."
      );
    }
    if (!deep.integridade_analise?.valuation_zone_suppressed && list(deep.integridade_analise?.convergence_provisional_layers).length) {
      writer.card(
        "Memória de cálculo parcial",
        `Faixa preservada a partir dos valores declarados em ${list(deep.integridade_analise.convergence_provisional_layers).join(", ")}.`,
        "Essas camadas permanecem identificadas como referências provisórias até a memória aritmética ser detalhada."
      );
    }
  }
  const groups = [
...
function sameModule(left, right, key) {
  const leftValue = left?.nexoModules?.[key] ?? null;
  const rightValue = right?.nexoModules?.[key] ?? null;
  return JSON.stringify(leftValue) === JSON.stringify(rightValue);
}

function writeFinal(writer, final, priorStage = null) {
  if (!final) {
  ...
  if (price.zona_convergencia || price.besst || price.margem_seguranca || price.observacao) {
    // Keep the final price summary with the conclusion. When space is tight,
    // moving the complete block prevents an almost-empty trailing page.
    writer.ensure(190);
    writer.subheading("Preço final");
    writer.pair("Zona de convergência", price.zona_convergencia);
    writer.pair("BESST", price.besst);
    const hasConsolidatedZone = !/^(?:N\/D|Faixa não consolidada)/i.test(asText(price.zona_convergencia));
    writer.pair(hasConsolidatedZone ? "Margem de segurança" : "Desconto aparente", price.margem_seguranca);
  ...
  if (!priorStage || !sameModule(final, priorStage, "HDL") || final.hdl_conclusao !== priorStage.hdl_conclusao) {
    writeHdl(writer, final, { showUnavailable: true });
  }
  if (!priorStage || !sameModule(final, priorStage, "NFI")) writeNfi(writer, final, { showUnavailable: true });
  if (!priorStage || !sameModule(final, priorStage, "TDN") || final.tdn_conclusao !== priorStage.tdn_conclusao) {
    writeTdn(writer, final, { showUnavailable: true });
  }
  if (!priorStage || !sameModule(final, priorStage, "EDG")) writeEdg(writer, final);
}
...
export function renderNexoReportPdf(payload = {}) {
  return new Promise((resolve, reject) => {
    ...
    // Prefer starting the final audit as one substantial block instead of
    // orphaning only its conclusion on a nearly empty last page.
    writer.ensure(420);
    writer.section("4", "Reclassificação final");
    const priorStage = list(payload.deepAdds).at(-1) || payload.deep || null;
    writeFinal(writer, payload.final, priorStage);
