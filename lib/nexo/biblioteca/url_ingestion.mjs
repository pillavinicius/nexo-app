...
import { parseDocument } from "./document_parser.mjs";
...
export async function ingestUserSource({ repository, ticker, assetType = "acao-br", sourceUrl, fetchImpl = fetch, lookupImpl = lookup, now = () => new Date() }) {
  ...
  if (existing?.status_parse === "ok") return { inserted: false, dedupKey, status: "ok", alreadyExisting: true };
  ...
  const parsed = await parseDocument({ content: downloaded.content, formato: downloaded.formato });
  if (parsed.status !== "ok") throw new Error(parsed.erro || "biblioteca_source_parse_failed");

  const stored = existing ? { inserted: false } : await repository.insertIngestedDocument({
    dedupKey,
    ...
    metadata: { supplied_by_user: true, requested_ticker: asset.ticker },
  ...
  return { inserted: stored.inserted, dedupKey, status: "ok", alreadyExisting: Boolean(existing), characters: parsed.texto.length };
}
