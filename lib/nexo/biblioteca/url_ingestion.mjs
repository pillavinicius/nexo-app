...
async function atStage(stage, operation) {
  try {
    ...
    if (String(error?.message || "").startsWith("biblioteca_")) throw error;
    if (stage === "parse" && /fake worker|pdf\.worker|worker.*module/i.test(String(error?.message || error))) {
      throw new Error("biblioteca_source_parser_runtime_unavailable", { cause: error });
    }
    const wrapped = new Error(`biblioteca_source_${stage}_failed`);
    wrapped.cause = error;
    throw wrapped;
  }
}
...
export async function ingestUserSource({ repository, ticker, assetType = "acao-br", sourceUrl, relevanceTerms = [], fetchImpl = fetch, lookupImpl = lookup, now = () => new Date() }) {
  ...
  let asset = await atStage("asset_lookup", () => repository.findAssetByTicker(ticker));
  if (!asset) asset = await atStage("asset_lookup", () => repository.ensureProvisionalAsset({ ticker, classe: assetType }));
  ...
  const existing = await atStage("database", () => repository.findByDedupKey(dedupKey));
  let existingMetadata = existing?.metadata_json || {};
  if (typeof existingMetadata === "string") {
    try { existingMetadata = JSON.parse(existingMetadata || "{}"); } catch { existingMetadata = {}; }
  }
  ...
  if (existing?.status_parse === "ok" && existing?.parser_version === B3_PARSER_VERSION && existingMetadata.raw_binary_persisted === true) {
    return {
      inserted: false, dedupKey, status: "ok", alreadyExisting: true,
      pageCount: Number(existingMetadata.page_count || 0), chunkCount: Number(existingMetadata.chunk_count || 0),
      rawBinaryPersisted: true,
    };
  }

  const downloaded = await atStage("download", () => fetchPublicDocument(validated.toString(), { fetchImpl, lookupImpl }));
  ...
  const baseMetadata = {
    ...
    raw_binary_persisted: true,
  };
  await atStage("database", () => repository.upsertRawDocument({
    dedupKey,
    ...
    dataDocumento: existing?.data_documento || now().toISOString().slice(0, 10),
    urlOrigem: downloaded.url,
    formato: downloaded.formato,
    metadata: baseMetadata,
    conteudo: downloaded.content,
    contentTypeDeclarado: downloaded.contentTypeDeclarado,
    hashConteudo,
  }));
  const parsed = await atStage("parse", () => parseDocument({ content: downloaded.content, formato: downloaded.formato, relevanceTerms: effectiveTerms }));
  if (parsed.status !== "ok") throw new Error(parsed.erro || "biblioteca_source_parse_failed");

  const stored = await atStage("database", () => repository.upsertParsedDocument({
    dedupKey,
    issuerId: asset.issuer_id,
    fonte: "ri",
    sourceDocumentId,
    ...
    metadata: {
      ...
      full_text_characters: parsed.fullTextCharacters,
  ...
  const indexed = await atStage("database", () => repository.replaceDocumentIndex({
    dedupKey,
    pages: parsed.pages,
  ...
  return {
    ...
    pageCount: Number(indexed.pages || parsed.pageCount || 0),
    chunkCount: Number(indexed.chunks || parsed.chunkCount || 0),
    rawBinaryPersisted: true,
  };
}
