import { createHash } from "node:crypto";

export const B3_PARSER_VERSION = "BIB_B3_2_PARSER_v2.1";
export const MAX_CONTEXT_DOCUMENT_CHARS = 18_000;
export const MAX_CHUNK_CHARS = 1_800;
export const CHUNK_OVERLAP_CHARS = 180;
const MAX_RELEVANT_PAGES = 12;
const TABLE_ROW_Y_TOLERANCE = 5;

function normalizeText(value) {
  return String(value || "")
    .replace(/\u0000/g, "")
    .replace(/[ \t]+\n/g, "\n")
    .replace(/\n[ \t]+/g, "\n")
    .replace(/\n{3,}/g, "\n\n")
    .replace(/[ \t]{2,}/g, " ")
    .trim();
}

function decodeEntities(value) {
  const entities = { amp: "&", lt: "<", gt: ">", quot: '"', apos: "'", nbsp: " " };
  return value.replace(/&(#x?[0-9a-f]+|[a-z]+);/gi, (match, token) => {
    if (token[0] === "#") {
      const hex = token[1]?.toLowerCase() === "x";
      const code = Number.parseInt(token.slice(hex ? 2 : 1), hex ? 16 : 10);
      return Number.isFinite(code) ? String.fromCodePoint(code) : match;
    }
    return entities[token.toLowerCase()] ?? match;
  });
}

export function extractHtmlText(content) {
  const html = Buffer.from(content).toString("utf8");
  return normalizeText(decodeEntities(html
    .replace(/<(script|style|noscript|svg)\b[^>]*>[\s\S]*?<\/\1>/gi, " ")
    .replace(/<\s*br\s*\/?>/gi, "\n")
    .replace(/<\/(p|div|li|tr|h[1-6]|section|article)>/gi, "\n")
    .replace(/<[^>]+>/g, " ")));
}

export function extractTableRows(items, pageNumber) {
  const lines = [];
  const positionedItems = items.map((item) => ({
    value: String(item.str || "").trim(),
    x: Number(item.transform?.[4] || 0),
    y: Number(item.transform?.[5] || 0),
  })).filter((item) => item.value)
    .sort((a, b) => b.y - a.y || a.x - b.x);

  for (const item of positionedItems) {
    const line = lines.reduce((nearest, candidate) => {
      const distance = Math.abs(candidate.y - item.y);
      if (distance > TABLE_ROW_Y_TOLERANCE || (nearest && nearest.distance <= distance)) return nearest;
      return { candidate, distance };
    }, null)?.candidate;
    if (!line) {
      lines.push({ y: item.y, cells: [{ x: item.x, value: item.value }] });
      continue;
    }
    line.cells.push({ x: item.x, value: item.value });
  }
  const candidates = lines
    .sort((a, b) => b.y - a.y)
    .map((line) => line.cells.sort((a, b) => a.x - b.x).map((cell) => cell.value))
    .filter((cells) => cells.length >= 3 && cells.some((cell) => /\d/.test(cell)));
  return candidates.length >= 2 ? [{ page: pageNumber, rows: candidates.slice(0, 80) }] : [];
}

async function extractPdfContent(content) {
  // pdf.js normally resolves its fake worker through a relative dynamic import.
  // Serverless bundlers cannot reliably trace that variable import, so the
  // deployed function may contain pdf.mjs but omit pdf.worker.mjs. Load the
  // worker explicitly and register it before opening the document.
  const [{ getDocument }, { WorkerMessageHandler }] = await Promise.all([
    import("pdfjs-dist/legacy/build/pdf.mjs"),
    import("pdfjs-dist/legacy/build/pdf.worker.mjs"),
  ]);
  globalThis.pdfjsWorker = { WorkerMessageHandler };
  const loadingTask = getDocument({
    data: new Uint8Array(content),
    disableFontFace: true,
    isEvalSupported: false,
    useSystemFonts: true,
  });
  const pdf = await loadingTask.promise;
  const pages = [];
  try {
    for (let number = 1; number <= pdf.numPages; number += 1) {
      const page = await pdf.getPage(number);
      const text = await page.getTextContent();
      pages.push({
        number,
        text: normalizeText(text.items.map((item) => item.str || "").join(" ")),
        tables: extractTableRows(text.items, number),
      });
      page.cleanup();
    }
    return pages;
  } finally {
    await loadingTask.destroy();
  }
}

function chunkSection(text) {
  const first = normalizeText(text).split(/(?<=[.!?])\s+/)[0] || "";
  return first.slice(0, 140) || null;
}

export function buildDocumentChunks(pages = [], { maxChars = MAX_CHUNK_CHARS, overlapChars = CHUNK_OVERLAP_CHARS } = {}) {
  const safeMax = Math.max(400, Number(maxChars) || MAX_CHUNK_CHARS);
  const safeOverlap = Math.min(Math.max(0, Number(overlapChars) || 0), Math.floor(safeMax / 3));
  const chunks = [];
  for (const page of pages) {
    const text = normalizeText(page?.text);
    if (!text) continue;
    let offset = 0;
    while (offset < text.length) {
      let end = Math.min(text.length, offset + safeMax);
      if (end < text.length) {
        const boundary = text.lastIndexOf(" ", end);
        if (boundary > offset + Math.floor(safeMax * 0.65)) end = boundary;
      }
      const content = normalizeText(text.slice(offset, end));
      if (content) {
        chunks.push({
          ordem: chunks.length,
          paginaInicio: Number(page.number) || 1,
          paginaFim: Number(page.number) || 1,
          secao: chunkSection(content),
          texto: content,
          tamanhoCaracteres: content.length,
          hashTexto: createHash("sha256").update(content).digest("hex"),
        });
      }
      if (end >= text.length) break;
      offset = Math.max(offset + 1, end - safeOverlap);
    }
  }
  return chunks;
}

function searchTokens(values) {
  return [...new Set((Array.isArray(values) ? values : [values])
    .flatMap((value) => String(value || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().match(/[a-z0-9]{3,}/g) || [])
    .filter((token) => !["para", "pela", "pelo", "como", "qual", "quais", "sobre", "carteira", "documento", "fonte"].includes(token))
  )];
}

function expandSearchTokens(tokens) {
  const expanded = new Set(tokens);
  if (tokens.includes("inadimplencia")) ["inad", "npl", "cobertura", "credito", "perda"].forEach((token) => expanded.add(token));
  if (tokens.includes("roic")) ["roe", "rentabilidade", "retorno", "patrimonio"].forEach((token) => expanded.add(token));
  return [...expanded];
}

export function selectRelevantPdfContent(pages, tables = [], relevanceTerms = []) {
  const queries = (Array.isArray(relevanceTerms) ? relevanceTerms : [relevanceTerms])
    .map((term) => expandSearchTokens(searchTokens(term)))
    .filter((tokens) => tokens.length);
  const tokens = [...new Set(queries.flat())];
  if (!tokens.length) {
    return {
      texto: normalizeText(pages.map((page) => page.text).join("\n\n")).slice(0, MAX_CONTEXT_DOCUMENT_CHARS),
      tabelas: tables.slice(0, 40),
      relevantPageNumbers: [],
    };
  }

  const normalizedPages = pages.map((page) => ({
    ...page,
    normalized: page.text.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase(),
  }));
  const documentFrequency = new Map(tokens.map((token) => [
    token,
    normalizedPages.filter((page) => page.normalized.includes(token)).length,
  ]));
  const perQueryLimit = Math.max(2, Math.ceil(MAX_RELEVANT_PAGES / queries.length));
  const ranked = [];
  for (const queryTokens of queries) {
    const queryRanking = normalizedPages.map((page) => {
      const matches = queryTokens.filter((token) => page.normalized.includes(token));
      const rarityScore = matches.reduce((score, token) => score + Math.log((pages.length + 1) / ((documentFrequency.get(token) || 0) + 1)) + 1, 0);
      const coverage = matches.length / queryTokens.length;
      return { number: page.number, text: page.text, score: rarityScore + coverage * 5, matches: matches.length };
    }).filter((page) => page.matches > 0)
      .sort((a, b) => b.score - a.score || b.matches - a.matches || a.number - b.number)
      .slice(0, perQueryLimit);
    for (const page of queryRanking) if (!ranked.some((selected) => selected.number === page.number)) ranked.push(page);
  }
  ranked.splice(MAX_RELEVANT_PAGES);

  const selected = ranked.length ? ranked : pages.slice(0, MAX_RELEVANT_PAGES);
  const relevantPageNumbers = selected.map((page) => page.number);
  const relevantText = selected.map((page) => `[Página ${page.number}]\n${page.text}`).join("\n\n");
  const intro = pages.slice(0, 3).map((page) => `[Página ${page.number}]\n${page.text}`).join("\n\n").slice(0, 2_500);
  return {
    texto: normalizeText(`TRECHOS SELECIONADOS PARA AS LACUNAS:\n${relevantText}\n\nINÍCIO DO DOCUMENTO:\n${intro}`).slice(0, MAX_CONTEXT_DOCUMENT_CHARS),
    tabelas: tables.filter((table) => relevantPageNumbers.includes(table.page)).slice(0, 40),
    relevantPageNumbers,
  };
}

export async function parseDocument({ content, formato, relevanceTerms = [] }) {
  const bytes = Buffer.from(content || []);
  if (!bytes.length) throw new Error("biblioteca_parse_content_required");
  let texto = "";
  let tabelas = [];
  let relevantPageNumbers = [];
  let pages = [];
  if (formato === "pdf") {
    pages = await extractPdfContent(bytes);
    const allTables = pages.flatMap((page) => page.tables || []);
    ({ texto, tabelas, relevantPageNumbers } = selectRelevantPdfContent(pages, allTables, relevanceTerms));
  }
  else if (["html", "text", "xml", "json"].includes(formato)) {
    const fullText = formato === "html" ? extractHtmlText(bytes) : normalizeText(bytes.toString("utf8"));
    pages = [{ number: 1, text: fullText, tables: [] }];
    texto = fullText.slice(0, MAX_CONTEXT_DOCUMENT_CHARS);
  }
  else return { status: "nao_suportado", texto: null, tabelas: [], parserVersion: B3_PARSER_VERSION, erro: `formato_${formato}_nao_suportado` };
  if (!texto) return { status: "falhou", texto: null, tabelas: [], parserVersion: B3_PARSER_VERSION, erro: "biblioteca_parse_text_empty" };
  const normalizedPages = pages.map((page) => ({
    pagina: Number(page.number) || 1,
    texto: normalizeText(page.text),
    tabelas: Array.isArray(page.tables) ? page.tables : [],
    hashTexto: createHash("sha256").update(normalizeText(page.text)).digest("hex"),
  })).filter((page) => page.texto);
  const chunks = buildDocumentChunks(pages);
  return {
    status: "ok",
    texto: texto.slice(0, MAX_CONTEXT_DOCUMENT_CHARS),
    tabelas,
    pages: normalizedPages,
    chunks,
    pageCount: normalizedPages.length,
    chunkCount: chunks.length,
    fullTextCharacters: normalizedPages.reduce((total, page) => total + page.texto.length, 0),
    relevantPageNumbers,
    parserVersion: B3_PARSER_VERSION,
    erro: null,
  };
}

export async function parsePendingDocuments({ repository, limit = 20 }) {
  const pending = await repository.listPendingDocuments({ limite: limit });
  const result = { discovered: pending.length, parsed: 0, unsupported: 0, failed: 0 };
  for (const document of pending) {
    try {
      const raw = document.conteudo_binario || (typeof repository.readRawDocument === "function" ? await repository.readRawDocument(document.dedup_key) : null);
      const parsed = await parseDocument({ content: raw, formato: document.formato });
      await repository.updateParseState({
        dedupKey: document.dedup_key,
        status: parsed.status,
        texto: parsed.texto,
        tabelas: parsed.tabelas,
        hashConteudo: document.hash_conteudo,
        erro: parsed.erro,
        parserVersion: parsed.parserVersion,
      });
      if (parsed.status === "ok" && typeof repository.replaceDocumentIndex === "function") {
        await repository.replaceDocumentIndex({
          dedupKey: document.dedup_key,
          pages: parsed.pages,
          chunks: parsed.chunks,
          parserVersion: parsed.parserVersion,
        });
      }
      if (parsed.status === "ok") result.parsed += 1;
      else if (parsed.status === "nao_suportado") result.unsupported += 1;
      else result.failed += 1;
    } catch (error) {
      result.failed += 1;
      await repository.updateParseState({
        dedupKey: document.dedup_key,
        status: "falhou",
        hashConteudo: document.hash_conteudo,
        erro: String(error?.message || error).slice(0, 240),
        parserVersion: B3_PARSER_VERSION,
      });
    }
  }
  return result;
}

export async function reindexStoredDocuments({ repository, limit = 20 }) {
  if (typeof repository.listIndexBackfillCandidates !== "function") return { discovered: 0, indexed: 0, failed: 0 };
  const candidates = await repository.listIndexBackfillCandidates({ limite: limit });
  const result = { discovered: candidates.length, indexed: 0, failed: 0 };
  for (const document of candidates) {
    try {
      const raw = document.conteudo_binario || (typeof repository.readRawDocument === "function" ? await repository.readRawDocument(document.dedup_key) : null);
      const parsed = await parseDocument({ content: raw, formato: document.formato });
      if (parsed.status !== "ok") throw new Error(parsed.erro || "biblioteca_reindex_parse_failed");
      await repository.updateParseState({
        dedupKey: document.dedup_key,
        status: "ok",
        texto: parsed.texto,
        tabelas: parsed.tabelas,
        hashConteudo: document.hash_conteudo,
        parserVersion: parsed.parserVersion,
      });
      await repository.replaceDocumentIndex({
        dedupKey: document.dedup_key,
        pages: parsed.pages,
        chunks: parsed.chunks,
        parserVersion: parsed.parserVersion,
      });
      result.indexed += 1;
    } catch {
      result.failed += 1;
    }
  }
  return result;
}
