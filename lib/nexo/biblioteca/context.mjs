...
export function selectBibliotecaChunks(chunks = [], gaps = []) {
  ...
  for (const gap of expected) {
    const forGap = scored.filter((item) => item.matches.some((match) => match.gap === gap))
      .sort((a, b) => {
        const left = a.matches.find((match) => match.gap === gap)?.score || 0;
        const right = b.matches.find((match) => match.gap === gap)?.score || 0;
        return right - left || Number(b.userSource) - Number(a.userSource) || a.index - b.index;
      }).slice(0, 2);
