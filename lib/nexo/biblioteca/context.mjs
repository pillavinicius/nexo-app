...
export const B3_CONTEXT_VERSION = "BIB_B3_2_CONTEXT_v2.3";
...
function analysisStrings(value, path = "$", output = []) {
  if (typeof value === "string") {
    for (const segment of value.split(/(?<=[.!?;])\s+|(?<=%)\s*,\s*(?=[A-Za-zÀ-ÿ])|\n+/).map((item) => item.trim()).filter(Boolean)) {
...
export function findBibliotecaMetricConflicts(result, context) {
  ...
  if (!context?.available) return conflicts.slice(0, 8);
  const ledger = metricLedger(context);
  const populated = [...ledger.values()].filter((entry) => entry.values.length);
  if (populated.length < 2) return conflicts.slice(0, 8);
  for (const item of strings) {
    const normalizedText = gapKey(item.text);
...
export function suppressBibliotecaMetricConflicts(result, conflicts = []) {
  ...
  for (const conflict of asArray(conflicts)) {
    ...
    parent[key] = "Trecho suprimido pelo servidor: a associação entre a métrica e o valor não foi comprovada. A evidência permanece parcial ou aberta.";
    suppressed.push({
      path: conflict.path,
  ...
  if (suppressed.length) {
    governed.integridade_analise = {
      ...(governed.integridade_analise || {}),
      biblioteca_metric_conflicts_suppressed: suppressed,
    };
  }
  return governed;
}
