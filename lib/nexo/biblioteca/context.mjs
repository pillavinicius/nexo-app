...
function percentageTokens(value) {
  return new Set((String(value || "").match(/-?\d+(?:[.,]\d+)?\s*%/g) || [])
...
function documentSupportsAnswer(document, gap, answerText) {
  ...
  const answerValues = percentageTokens(answerText);
  if (!answerValues.size) return false;
  const documentValues = percentageTokens(document.text);
  const shared = [...answerValues].filter((token) => documentValues.has(token));
  return shared.length >= Math.min(2, answerValues.size);
}
...
export function applyBibliotecaAudit(result, context, { expectedGaps = [] } = {}) {
  ...
  const gaps = governedCandidates.map((item) => {
    ...
    const evidence = [...new Set([...explicitEvidence, ...inferredEvidence])];
    ...
    const retrievalVerified = inferredEvidence.length > 0;
    const resolved = evidence.length > 0 && !admitsIncompleteEvidence && (item?.status === "resolvida" || retrievalVerified);
    const partial = evidence.length > 0 && (item?.status === "parcial" || admitsIncompleteEvidence);
