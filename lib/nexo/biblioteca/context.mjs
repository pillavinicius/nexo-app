...
export const B3_CONTEXT_VERSION = "BIB_B3_2_CONTEXT_v2.4";
...
export function reconcileBibliotecaMetricConflicts(result, conflicts = []) {
  ...
  for (const conflict of asArray(conflicts)) {
    ...
    if (conflict.target_metric === "inadimplência sem janela definida") {
      corrected = corrected.replace(/\bINAD\b(?!\s*(?:\+?\s*(?:30|90)|New\s+NPL))/gi, "indicador de inadimplência sem janela definida");
    } else if (conflict.target_metric === "requerimento de capital sem decomposição") {
      corrected = corrected.replace(
        /m[ií]nimo(?:\s+regulat[oó]rio)?/gi,
        "referência regulatória citada, ainda sem decomposição entre mínimo-base, buffers e adicionais"
      );
