...
export function buildTdnPromptContext(tdn) {
  if (!tdn || tdn.status === "not_applicable") {
    return `TDN não aplicável: ${tdn?.note || "classe fora do TDN v1.0"}. Não substitua métricas setoriais, não invente score e não crie lacuna de TDN ou defesa nominal para este ativo.`;
  }
  if (tdn.status !== "ok") {
...
function referencesInapplicableTdn(value) {
  const normalized = String(value || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase();
  return /\btdn\b|teste de defesa nominal|defesa nominal/.test(normalized);
}

function suppressInapplicableTdnGaps(source, module) {
  if (module?.status !== "not_applicable") return source;
  const clean = { ...source };
  if (Array.isArray(source.lacunas_deep)) {
    clean.lacunas_deep = source.lacunas_deep.filter((gap) => !referencesInapplicableTdn(gap));
  }
  if (Array.isArray(source.lacunas)) {
    clean.lacunas = source.lacunas.filter((gap) => !referencesInapplicableTdn(gap?.q || gap?.lacuna || gap));
  }
  if (Array.isArray(source.lacunas_documentais)) {
    clean.lacunas_documentais = source.lacunas_documentais.filter((gap) => !referencesInapplicableTdn(gap?.lacuna || gap?.q || gap));
  }
  return clean;
}
...
export function applyTdnToAnalysis({ phase, result, tdn }) {
  ...
  const module = tdn || original?.nexoModules?.TDN || null;
  if (!module) return original;
  const source = suppressInapplicableTdnGaps(original, module);
