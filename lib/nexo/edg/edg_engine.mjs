...
export function buildEdgAnalysisContext(edg, record = {}) {
  const normalized = normalizeEdgeRecord(record);
  const result = edg || computeEDG(normalized);
  const lines = [
    `EDG REGISTRADO — ${result.version}`,
    `Tipo declarado: ${result.edge_type}; status registrado: ${result.edge_status}`,
  ];

  if (result.has_declared_edge) {
    lines.push(
      `Evidência declarada: ${normalized.edge_evidence}`,
      `Insumo NEXO: ${normalized.edge_insumo}`,
      `Condição observável: ${normalized.edge_expiry_condition}`,
      `Declarado em: ${normalized.edge_declared_at}`
    );
  }

  lines.push(
    "Produza o veredito analítico bruto sem aplicar os tetos D2 ou D3. A governança EDG será aplicada deterministicamente pelo servidor depois da sua resposta."
  );
...
export function applyEdgGuardrails({ phase, result, edg }) {
  ...
  governed.edg_governance = {
    ...
    changes,
    comparison: (() => {
      const field = classificationFieldsForPhase(phase)[0];
      if (!field) return null;
      return {
        field,
        without_edg: source[field] ?? null,
        with_edg: governed[field] ?? null,
      };
    })(),
  };
