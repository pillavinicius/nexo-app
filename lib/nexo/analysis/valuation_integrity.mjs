export const VALUATION_INTEGRITY_VERSION = "VALUATION_v1.0";
...
function structuredCalculation(layer) {
  ...
  if (formula === FORMULAS.TOTAL_PER_UNIT_MULTIPLE) {
    ...
    return {
      ...
      inputs: { multiplo: multiple, valor_total: total, quantidade_unidades: units },
      source: "structured",
    };
  }
  ...
  if (formula === FORMULAS.PER_UNIT_MULTIPLE) {
    ...
    return {
      ...
      inputs: { multiplo: multiple, valor_por_unidade: perUnit },
      source: "structured",
    };
  }
  ...
  if (multiple && totalPerUnits) {
    ...
    if (factor > 0 && total > 0 && units > 0) {
      return {
        ...
        inputs: { multiplo: factor, valor_total: total, quantidade_unidades: units },
        source: "legacy_text",
      };
    }
  }
...
function isMismatch(declared, expected) {
  ...
  return Math.abs(declared - expected) > Math.max(0.05, Math.abs(expected) * 0.02);
}
...
export function reconcileValuationLayers(layers = []) {
  ...
  return {
    ...
    zone_must_be_suppressed: changed.length > 0 || invalid.length > 0,
  };
}
