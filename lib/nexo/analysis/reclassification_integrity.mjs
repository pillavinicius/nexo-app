...
export const RECLASSIFICATION_INTEGRITY_VERSION = "P3B_v1.3";
...
export function reconcilePriceNarrative({ zone, besst, referencePrice, previousNarrative = "" }) {
  ...
  const discounts = besstPrices.map((price) => ((floor - price) / floor) * 100);
...
export function reconcileConvergenceZone({ zone, zoneCalculation, valuation }) {
  ...
  const verified = new Map(layerValues
    .filter((layer) => layer.status === "verified" && Number.isFinite(layer.value))
    .map((layer) => [layer.code, layer.value]));
  const included = [...new Set((Array.isArray(zoneCalculation?.camadas_incluidas)
  ...
  if (included.length < 2 || included.some((code) => !verified.has(code))) {
    return { value: zone, status: "invalid_contract", suppressed: true, included_layers: included };
  }

  const excludedVerified = [...verified.keys()].filter((code) => !included.includes(code));
  if (excludedVerified.length && !justification) {
    return {
      value: zone,
      status: "unjustified_exclusion",
      suppressed: true,
      included_layers: included,
      excluded_layers: excludedVerified,
    };
  }

  if (parsed.length !== 2) {
    return { value: zone, status: "invalid_bounds", suppressed: true, included_layers: included };
  }

  const expected = included.map((code) => verified.get(code));
  const expectedLow = Math.min(...expected);
  const expectedHigh = Math.max(...expected);
  const zoneLow = Math.min(...parsed);
  const zoneHigh = Math.max(...parsed);
  if (!valuesMatch(zoneLow, expectedLow) || !valuesMatch(zoneHigh, expectedHigh)) {
    return {
      value: zone,
  ...
  const corrected = parsed[0] > parsed[1] || String(zone || "").trim() !== normalizedValue;
  return {
    value: normalizedValue,
    status: corrected ? "normalized" : "verified",
    corrected,
    suppressed: false,
    included_layers: included,
    excluded_layers: excludedVerified,
    exclusion_reason: justification || null,
    expected_range: [round2(expectedLow), round2(expectedHigh)],
  };
}
...
export function reconcileDeepIntegrity(result, history = {}, { documentIds = [], referencePrice = null } = {}) {
  ...
  const valuationWarning = "Zona e BESST indisponíveis: a derivação de ao menos uma camada ou a memória estruturada da convergência não passou na validação. O servidor preservou somente os cálculos verificáveis e não inferiu uma faixa automaticamente.";
  const governedZone = zoneSuppressed
    ? "N/D — zona suprimida por integridade insuficiente das camadas ou da convergência"
    : convergence.value;
  const governedBesst = zoneSuppressed
    ? "N/D — faixa suprimida com a zona não verificável"
    : source.besst;
  ...
  return {
    ...
    preco_final: {
      ...
      observacao: isScan
        ...
        ? `Faixas preservadas do último Deep. Convergência baseada em ${stage.integridade_analise.convergence_included_layers.join(", ")}; ${stage.integridade_analise.convergence_excluded_layers.join(", ")} permaneceu fora da faixa central: ${stage.integridade_analise.convergence_exclusion_reason || "justificativa registrada no Deep"}.`
