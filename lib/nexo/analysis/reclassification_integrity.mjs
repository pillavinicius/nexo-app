...
export const RECLASSIFICATION_INTEGRITY_VERSION = "P3B_v1.8";
const MAX_CONVERGENCE_SPREAD_FROM_FLOOR_PCT = 35;
...
function reconcileExclusionReason(reason, valuation) {
  let governed = String(reason || "").trim();
  for (const layer of Array.isArray(valuation?.layers) ? valuation.layers : []) {
    const integrity = layer?.calculo_integridade;
    const code = normalizedLayerCode(layer?.c ?? layer?.camada ?? layer?.nome);
    const declared = finiteNumber(integrity?.declared_value);
    const calculated = finiteNumber(integrity?.calculated_value);
    if (!code || !Number.isFinite(declared) || !Number.isFinite(calculated) || round2(declared) === round2(calculated)) continue;
    const integer = Number.isInteger(declared);
    const declaredPattern = integer
      ? `${escapeRegExp(declared)}(?:[.,]0{1,2})?`
      : escapeRegExp(declared.toLocaleString("pt-BR", { minimumFractionDigits: 0, maximumFractionDigits: 2 })).replace(",", "[.,]");
    const pattern = new RegExp(`(\\b${code}\\b[^.;\\n]{0,120}?R\\$\\s*)${declaredPattern}`, "gi");
    governed = governed.replace(pattern, `$1${calculated.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`);
  }
  return governed;
}
...
export function reconcileConvergenceZone({ zone, zoneCalculation, valuation }) {
  ...
  const justification = reconcileExclusionReason(zoneCalculation?.justificativa_exclusoes, valuation);
  ...
  const dispersionPct = round2(((expectedHigh - expectedLow) / expectedLow) * 100);
  if (dispersionPct > MAX_CONVERGENCE_SPREAD_FROM_FLOOR_PCT) {
    return {
      value: zone,
      status: "divergent_layers",
      suppressed: true,
      included_layers: included,
      excluded_layers: [],
      exclusion_reason: `As camadas diferem ${dispersionPct.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}% sobre o piso, acima do limite de convergência de ${MAX_CONVERGENCE_SPREAD_FROM_FLOOR_PCT}%.`,
...
function reconcileThesisLayerPrices(value, layers = []) {
  let text = String(value || "");
  for (const layer of Array.isArray(layers) ? layers : []) {
    const integrity = layer?.calculo_integridade;
    const declared = finiteNumber(integrity?.declared_value);
    const calculated = finiteNumber(integrity?.calculated_value);
    if (!Number.isFinite(declared) || !Number.isFinite(calculated) || round2(declared) === round2(calculated)) continue;
    const declaredToken = declared.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    const calculatedToken = calculated.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    const pattern = new RegExp(`((?:R\\$|US\\$|USD|EUR|€)\\s*)${escapeRegExp(declaredToken).replace(",", "[.,]")}(?!\\d)`, "gi");
    text = text.replace(pattern, `$1${calculatedToken}`);
  }
  return text;
}

function reconcileThesisValuationClaims(value, { zoneSuppressed = false, priceNarrative = "" } = {}) {
  const source = String(value || "").trim();
  if (!source) return zoneSuppressed
    ? "A faixa de preço não foi consolidada pelo motor de integridade; zona e BESST permanecem indisponíveis até existirem pelo menos duas camadas válidas e convergentes."
    : String(priceNarrative || "").trim();
  const sentences = source.split(/(?<=[.!?])\s+(?=[A-ZÀ-Ý])/u).filter(Boolean);
  const valuationClaim = zoneSuppressed
    ? /\b(?:converg(?:e|em|iu|iram|ência)|zona\s+(?:de\s+)?(?:convergência|preço)|faixa\s+(?:de\s+)?preço|valor\s+justo|BESST|potencial\s+de\s+valoriza[cç][aã]o|upside|downside|margem\s+de\s+seguran[cç]a)\b/i
    : /\b(?:pre[cç]o\s+atual[^.!?]{0,180}(?:zona|BESST)|potencial\s+de\s+valoriza[cç][aã]o|upside|downside|BESST|margem\s+de\s+seguran[cç]a)\b/i;
  const kept = sentences.filter((sentence) => !valuationClaim.test(sentence));
  if (zoneSuppressed) {
    kept.push("A faixa de preço não foi consolidada pelo motor de integridade; zona e BESST permanecem indisponíveis até existirem pelo menos duas camadas válidas e convergentes.");
  } else if (String(priceNarrative || "").trim()) {
    kept.push(String(priceNarrative).trim());
  }
  return kept.join(" ");
}

export function sanitizeNextSteps(steps = []) {
  return (Array.isArray(steps) ? steps : []).map((step) => String(step || "").trim()).filter(Boolean).map((step) =>
    step.replace(
      /\b(?:reavaliar|alterar|elevar|mudar)\s+(?:o\s+)?veredito\s+(?:para|a)\s+COMPRAR\b/gi,
      "executar um novo fluxo completo do NEXO e então reavaliar o veredito"
    ).replace(
      /\b(?:classificar|reclassificar)\s+(?:o\s+ativo\s+)?(?:como|para)\s+COMPRAR\b/gi,
      "executar um novo fluxo completo do NEXO antes de qualquer reclassificação"
    )
  );
}

function compoundRealRate(macro = {}) {
  const nominal = finiteNumber(macro?.selic_target);
  const inflation = finiteNumber(macro?.ipca_12m);
  if (nominal === null || inflation === null || nominal <= -100 || inflation <= -100) return null;
  return round2((((1 + nominal / 100) / (1 + inflation / 100)) - 1) * 100);
}

function reconcileRealRateClaims(value, realRate) {
  if (!Number.isFinite(realRate)) return value;
  if (typeof value === "string") {
    return value.replace(
      /(Selic\s+real\s*(?:\(?\s*~?\s*)?)(\d+(?:[.,]\d+)?)(\s*%\s*\)?)/gi,
      (_, prefix, _declared, suffix) => `${prefix}${realRate.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}${suffix}`
    );
  }
  if (Array.isArray(value)) return value.map((item) => reconcileRealRateClaims(item, realRate));
  if (value && typeof value === "object") {
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, reconcileRealRateClaims(item, realRate)]));
  }
  return value;
}

export function reconcileDeepIntegrity(result, history = {}, { documentIds = [], referencePrice = null, macro = null } = {}) {
  const realRate = compoundRealRate(macro);
  const source = result && typeof result === "object" ? reconcileRealRateClaims(result, realRate) : {};
  ...
  const valuation = reconcileValuationLayers(source.preco, { documentIds });
  ...
  const valuationWarning = convergence.status === "divergent_layers"
    ? `Faixa de preço não consolidada: as camadas válidas não convergiram. ${convergence.exclusion_reason}`
    : "Faixa de preço não consolidada: menos de duas camadas apresentaram valores utilizáveis. O desconto aparente não pode ser tratado como margem de segurança sem uma referência de valor validada.";
  const governedZone = zoneSuppressed
    ? convergence.status === "divergent_layers"
      ? "Faixa não consolidada — camadas divergentes"
  ...
  const governedThesis = reconcileThesisValuationClaims(
    reconcileThesisLayerPrices(source.tese_final, valuation.layers),
    { zoneSuppressed, priceNarrative: priceNarrative.calculated ? priceNarrative.value : "" }
  );

  return {
    ...source,
    preco: valuation.layers,
    tese_final: governedThesis,
    zona: governedZone,
    zona_calc: {
      ...(source.zona_calc || {}),
    ...
    passos: sanitizeNextSteps(source.passos),
    ...
    integridade_analise: {
      ...
      convergence_dispersion_pct: convergence.dispersion_pct || null,
      ...
      macro_real_rate_pct: realRate,
      macro_real_rate_method: Number.isFinite(realRate) ? "compound_fisher" : null,
    },
  };
}
...
export function buildDeterministicFinal({ ticker, history = {} }) {
  ...
  return {
    ...
    riscos_incorporados: [],
    ajustes_score: [],
    tese_final: reconcileThesisValuationClaims(
      reconcileThesisLayerPrices(
        reconcileThesisMultiples(
      ...
      {
        zoneSuppressed: Boolean(stage.integridade_analise?.valuation_zone_suppressed),
        priceNarrative: stage.integridade_analise?.price_narrative_status === "server_calculated" ? stage.desconto : "",
      }
    ),
    preco_final: {
      zona_convergencia: isScan ? "N/D — Deep não realizado" : stage.zona || "N/D",
      besst: isScan ? "N/D — Deep não realizado" : stage.besst || "N/D",
      margem_seguranca: isScan
        ? "N/D — Deep não realizado"
        : stage.integridade_analise?.valuation_zone_suppressed
        ? "Desconto aparente não calculável sem uma faixa de preço consolidada."
        : stage.desconto || "N/D",
      observacao: isScan
        ? "Finalização baseada somente no Scan; nenhum valuation adicional foi inferido."
        : stage.integridade_analise?.valuation_zone_suppressed
        ? `Faixa não consolidada pelo motor de integridade: ${cleanSentence(stage.integridade_analise?.convergence_exclusion_reason || "faltaram duas camadas convergentes e verificáveis")}.`
    ...
    proximos_passos: Array.isArray(stage.passos)
      ? sanitizeNextSteps(stage.passos)
      : Array.isArray(scan.lacunas_deep)
      ? sanitizeNextSteps(scan.lacunas_deep)
      : [],
