...
export const RECLASSIFICATION_INTEGRITY_VERSION = "P3B_v1.2";
...
export function reconcileDeepIntegrity(result, history = {}, { documentIds = [], referencePrice = null } = {}) {
  ...
  const valuationWarning = "Zona e BESST indisponíveis: ao menos uma camada de preço apresentou valor incompatível com a fórmula declarada ou entradas insuficientes para validá-la. O servidor preservou ou corrigiu somente os cálculos verificáveis; a convergência não foi inferida automaticamente.";
  const governedZone = valuation.zone_must_be_suppressed
    ? "N/D — zona suprimida por inconsistência aritmética nas camadas de preço"
    : source.zona;
  const governedBesst = valuation.zone_must_be_suppressed
    ? "N/D — faixa suprimida com a zona inconsistente"
    : source.besst;
  ...
  const priceNarrative = reconcilePriceNarrative({
    ...
    besst: besst.value,
    referencePrice,
    previousNarrative: valuation.zone_must_be_suppressed ? valuationWarning : source.desconto,
  });
  ...
  return {
    ...
    integridade_analise: {
      ...
      valuation_zone_suppressed: valuation.zone_must_be_suppressed,
