...
export const RECLASSIFICATION_INTEGRITY_VERSION = "P3B_v1.7";
...
function reconcileThesisValuationClaims(value, { zoneSuppressed = false } = {}) {
  const source = String(value || "").trim();
  if (!source || !zoneSuppressed) return source;
  const sentences = source.split(/(?<=[.!?])\s+(?=[A-ZÀ-Ý])/u).filter(Boolean);
  const valuationClaim = /\b(?:converg(?:e|em|iu|iram|ência)|zona\s+(?:de\s+)?(?:convergência|preço)|faixa\s+(?:de\s+)?preço|valor\s+justo|BESST|potencial\s+de\s+valoriza[cç][aã]o)\b/i;
...
export function reconcileDeepIntegrity(result, history = {}, { documentIds = [], referencePrice = null, macro = null } = {}) {
  ...
  const valuationWarning = convergence.status === "divergent_layers"
    ...
    : "Faixa de preço não consolidada: menos de duas camadas apresentaram valores utilizáveis. Revise a memória de cálculo antes de interpretar margem de segurança.";
