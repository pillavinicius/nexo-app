export const TEXT_INTEGRITY_VERSION = "TEXT_v1.1";
  ...
  function visit(value, path = "$") {
    ...
    for (const [key, item] of Object.entries(value)) {
      ...
      if (typeof item === "string") {
        ...
        corrected = corrected
          .replace(/\bacima\s+dos?\s+refer[eê]ncia\s+regulat[oó]ria\s+citada\b/gi, "acima das referências regulatórias citadas")
          .replace(/\bacima\s+das?\s+refer[eê]ncia\s+regulat[oó]ria\s+citada\b/gi, "acima das referências regulatórias citadas")
          .replace(/\badicionaiss\b/gi, "adicionais");
        if (corrected !== item) {
          value[key] = corrected;
