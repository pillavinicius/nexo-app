export const dynamic = "force-dynamic";
export const maxDuration = 60;

function maskKey(url, key) {
  if (!key) return url;
  return url.replaceAll(key, "***");
}

function normalizeTicker(raw) {
  return String(raw || "AAPL").trim().toUpperCase();
}

async function fetchText(url) {
  const response = await fetch(url, {
    method: "GET",
    cache: "no-store",
  });

  const text = await response.text();

  let json = null;
  try {
    json = JSON.parse(text);
  } catch {
    json = null;
  }

  return {
    status: response.status,
    ok: response.ok,
    json,
    textPreview: text.slice(0, 500),
  };
}

function makeUrl(endpoint, params) {
  const url = new URL(`https://api.twelvedata.com/${endpoint}`);

  Object.entries(params).forEach(([key, value]) => {
    if (value !== undefined && value !== null && value !== "") {
      url.searchParams.set(key, String(value));
    }
  });

  return url.toString();
}

function isTwelveError(json) {
  if (!json) return false;

  if (json.status === "error") return true;
  if (json.code && Number(json.code) >= 400) return true;
  if (json.message && String(json.message).toLowerCase().includes("api")) {
    return true;
  }

  return false;
}

function summarize(endpoint, result, url, apiKey) {
  const json = result.json;

  const available =
    result.ok &&
    json &&
    !isTwelveError(json) &&
    Object.keys(json || {}).length > 0;

  return {
    endpoint,
    available,
    httpStatus: result.status,
    requestUrl: maskKey(url, apiKey),
    keys: json && typeof json === "object" ? Object.keys(json).slice(0, 30) : [],
    error:
      !available
        ? json?.message ||
          json?.status ||
          json?.code ||
          result.textPreview ||
          "Indisponível"
        : null,
    sample: json || result.textPreview,
  };
}

export async function GET(request) {
  try {
    const { searchParams } = new URL(request.url);

    const ticker = normalizeTicker(searchParams.get("ticker") || "AAPL");
    const apiKey = process.env.TWELVEDATA_API_KEY;

    if (!apiKey) {
      return Response.json(
        {
          ok: false,
          error: "TWELVEDATA_API_KEY não encontrada",
        },
        { status: 500 }
      );
    }

    const baseParams = {
      apikey: apiKey,
      symbol: ticker,
      format: "JSON",
    };

    const tests = [
      {
        name: "price",
        endpoint: "price",
        params: {
          ...baseParams,
        },
      },
      {
        name: "quote",
        endpoint: "quote",
        params: {
          ...baseParams,
        },
      },
      {
        name: "time_series",
        endpoint: "time_series",
        params: {
          ...baseParams,
          interval: "1day",
          outputsize: 250,
        },
      },
      {
        name: "profile",
        endpoint: "profile",
        params: {
          ...baseParams,
        },
      },
      {
        name: "statistics",
        endpoint: "statistics",
        params: {
          ...baseParams,
        },
      },
      {
        name: "income_statement",
        endpoint: "income_statement",
        params: {
          ...baseParams,
        },
      },
      {
        name: "balance_sheet",
        endpoint: "balance_sheet",
        params: {
          ...baseParams,
        },
      },
      {
        name: "cash_flow",
        endpoint: "cash_flow",
        params: {
          ...baseParams,
        },
      },
      {
        name: "dividends",
        endpoint: "dividends",
        params: {
          ...baseParams,
        },
      },
      {
        name: "earnings",
        endpoint: "earnings",
        params: {
          ...baseParams,
        },
      },
      {
        name: "etf",
        endpoint: "etf",
        params: {
          ...baseParams,
        },
      },
    ];

    const results = [];

    for (const test of tests) {
      const url = makeUrl(test.endpoint, test.params);

      try {
        const result = await fetchText(url);
        results.push(summarize(test.name, result, url, apiKey));
      } catch (error) {
        results.push({
          endpoint: test.name,
          available: false,
          error: error.message,
          requestUrl: maskKey(url, apiKey),
        });
      }

      await new Promise((resolve) => setTimeout(resolve, 1200));
    }

    return Response.json({
      ok: true,
      provider: "Twelve Data",
      ticker,
      updatedAt: new Date().toISOString(),
      summary: results.map((r) => ({
        endpoint: r.endpoint,
        available: r.available,
        httpStatus: r.httpStatus,
        error: r.error,
        keys: r.keys,
      })),
      results,
    });
  } catch (error) {
    return Response.json(
      {
        ok: false,
        error: error.message,
      },
      { status: 500 }
    );
  }
}
