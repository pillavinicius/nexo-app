...
const DEEP_S =
  '{"ticker":"","veredito_final":"COMPRAR|MONITORAR|AGUARDAR|EVITAR","score_original":0,"score_revisado":0,"score_max":30,"mudanca_score":"","ajustes_score":[{"dimensao":"","antes":0,"depois":0,"motivo":"","fonte_nova":""}],"lacunas":[{"q":"","r":""}],"lacunas_documentais":[{"lacuna":"","status":"resolvida|parcial|aberta","evidencia_documental":["source:id"]}],"preco":[{"c":"C1","vj":"","met":"","prem":"","calc":{"formula":"TOTAL_POR_UNIDADE_X_MULTIPLO|VALOR_POR_UNIDADE_X_MULTIPLO|MEDIA_PONDERADA_X_MULTIPLO|RENDA_POR_YIELD|AJUSTE_PERCENTUAL_CAMADA|NAO_VERIFICAVEL","multiplo":null,"valor_total":null,"quantidade_unidades":null,"valor_por_unidade":null,"renda_por_unidade":null,"yield_pct":null,"ajuste_pct":null,"camada_base":"","direcao":"DESCONTO|PREMIO","componentes":[{"rotulo":"","valor":0,"peso_pct":0}]}},{"c":"C2","vj":"","met":"","prem":"","calc":{"formula":"NAO_VERIFICAVEL"}},{"c":"C3","vj":"","met":"","prem":"","calc":{"formula":"NAO_VERIFICAVEL"}}],"valuations_classicos":[{"modelo":"Graham|Peter Lynch|Buffett moderno|Bazin","valor_justo":"","metodologia":"","premissas":""}],"tese_final":"","zona":"","zona_calc":{"camadas_incluidas":["C1","C2"],"justificativa_exclusoes":""},"besst":"","desconto":"","hdl_conclusao":"","tdn_conclusao":"","macro":[{"s":"","i":""}],"catalisadores":[{"d":"","p":""}],"riscos":[{"d":"","sev":"ALTO|MEDIO|BAIXO","g":""}],"passos":[""]}';

const DEEP_INTEGRITY_RULES =
  " Score original must reproduce the previous stage score. Only include ajustes_score for NEW evidence found in this Deep; never penalize a risk already counted in the previous stage. Each adjustment must use a 0-5 dimension score, explain the new evidence and cite its exact Biblioteca document ID in fonte_nova when documentary. The server will ignore your arithmetic and calculate the total from the adjustments. Fill lacunas_documentais for every investigated gap: resolved requires evidence for every requested component; use partial when at least one component is supported but another remains missing; use open when there is no sufficient answer. Referencing a figure or table without retrieving the requested values is partial, never resolved. Every resolved or partial gap must cite at least one exact available document ID. Preserve exact metric semantics and windows: INAD/NPL +90d, New NPL formation and their coverage ratios are distinct and may never be relabeled. Novo Mercado, Nível 1 and Nível 2 are mutually exclusive B3 listing segments; never combine them. Do not state a regulatory minimum without distinguishing the base requirement from buffers and institution-specific add-ons. Every C1/C2/C3 layer must include calc. Use numeric inputs in absolute base units, never scaled text such as bi or mi inside calc. The displayed vj must equal the declared formula result. If a unit base comes from an average or blend, use MEDIA_PONDERADA_X_MULTIPLO and disclose every component and weights totaling 100; never hide a haircut or call an unexplained value a conservative average. Use NAO_VERIFICAVEL only when the method cannot be expressed by the supported formulas; never invent a value to make the zone fit. zona_calc must name at least two included C1/C2/C3 layers; its bounds must equal the minimum and maximum included values. Any excluded valid layer requires an explicit methodological justification. Use N/D when there is no honest convergence. BESST is calculated deterministically by the server as the full 15-25% safety band below the lower bound of the convergence zone; do not improvise a narrow or reversed interval. tese_final must reconcile updated Deep inputs and must not repeat stale Scan multiples. The server independently verifies derivation, layer arithmetic, convergence and BESST, suppressing zone/BESST when required. hdl_conclusao is mandatory for Brazilian assets and must interpret the immutable server-calculated HDL values; never recalculate them. When the server supplies a valid TDN, tdn_conclusao is mandatory and must interpret it without recalculating any metric. TDN never changes the global score or verdict automatically.";
